		...
		if relayInfo.ConsumeQuota && taskErr == nil {
			...
			if quota != 0 {
				...
				model.RecordConsumeLog(ctx, relayInfo.UserId, relayInfo.ChannelId, 0, 0,
					modelName, tokenName, quota, logContent, relayInfo.TokenId, userQuota, 0, false, relayInfo.Group, other, c.ClientIP(), c.GetString(common.RequestIdKey), relayInfo.TokenGroup)
