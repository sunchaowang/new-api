	...
	defer func(ctx context.Context) {
		...
		if relayInfo.ConsumeQuota && taskErr == nil {
			...
			if quota != 0 {
				...
				logContent := fmt.Sprintf("模型固定价格 %v，分组倍率 %v，操作 %s", modelPrice, groupRatio, relayInfo.Action)
				other := make(map[string]interface{})
				other["model_price"] = modelPrice
				other["group_ratio"] = groupRatio
				model.RecordConsumeLog(ctx, relayInfo.UserId, relayInfo.ChannelId, 0, 0, modelName, tokenName, quota, logContent, relayInfo.TokenId, userQuota, 0, false, other, c.ClientIP(), c.GetString(common.RequestIdKey), relayInfo.TokenGroup)
