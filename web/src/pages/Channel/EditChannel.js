...
import { SideSheet, Space, Spin, Button, Tooltip, Input, Typography, Select, TextArea, Checkbox, Banner } from '@douyinfe/semi-ui';
...
const EditChannel = (props) => {
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'type') {
      ...
      switch (value) {
        case 2:
          localModels = ['mj_imagine', 'mj_variation', 'mj_reroll', 'mj_blend', 'mj_upscale', 'mj_describe', 'mj_uploads'];
          break;
        case 5:
          localModels = [
            'swap_face',
  ...
  useEffect(() => {
    ...
    localModelOptions = localModelOptions.filter((option, index, self) => index === self.findIndex((t) => t.label === option.label));
  ...
  return (
    <>
      <SideSheet
        ...
        title={isEdit ? t('更新渠道信息') : t('创建新的渠道')}
        ...
        <Spin spinning={loading}>
          ...
          {inputs.type === 8 && (
            <>
              ...
              <div style={{ marginTop: 10 }}>
                <Typography.Text strong>{t('完整的 Base URL，支持变量{model}')}：</Typography.Text>
              </div>
              <Input
                name="base_url"
          ...
          {inputs.type === 36 && (
            <>
              <div style={{ marginTop: 10 }}>
                <Typography.Text strong>{t('注意非Chat API，请务必填写正确的API地址，否则可能导致无法使用')}</Typography.Text>
              </div>
              <Input
                name="base_url"
          ...
          {!isEdit && (
            <div style={{ marginTop: 10, display: 'flex' }}>
              <Space>
                <Checkbox checked={batch} label={t('批量创建')} name="batch" onChange={() => setBatch(!batch)} />
          ...
          <div style={{ marginTop: 10, display: 'flex' }}>
            <Space>
              ...
              />
              <Typography.Text strong>{t('是否自动禁用（仅当自动禁用开启时有效），关闭后不会自动禁用该渠道：')}</Typography.Text>
            </Space>
          </div>
          <div style={{ marginTop: 10 }}>
            <Typography.Text strong>{t('状态码复写（仅影响本地判断，不修改返回到上游的状态码）')}：</Typography.Text>
          </div>
          <TextArea
            placeholder={
          ...
          <div style={{ marginTop: 10 }}>
            <Typography.Text strong>Proxy代理</Typography.Text>
          </div>
          <Input
            label="Proxy代理"
            name="proxy"
